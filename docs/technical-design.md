...
Inno Setup has no separate repair mode. Re-running `setup.exe` over an existing installation is the repair, and it re-applies files, icons, registry values, and the post-install sequence. The desktop shortcut is the one icon a re-run treats differently, because it is the only one a person arranges by hand. Rewriting the `.url` file makes Explorer place the icon in the first free desktop slot, so an upgrade would silently move a shortcut away from where its owner left it. The icon entry is therefore created only when the desktop has none; an existing shortcut is repointed in place afterwards, and only the keys whose values actually changed are written, so an upgrade on an unchanged port does not touch the file at all. Removal is still the other case a re-run must handle: the shortcut is optional and its URL carries the port, so a run that leaves the task unticked deletes it instead of stranding a shortcut for the previous port. Because the port page is always shown with the previous value pre-filled, one code path covers installation, upgrade, and repair, including reconfiguring the port when the current one prevents UI startup.
